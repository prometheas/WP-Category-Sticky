...
class bz_category_sticky
{
    ...
    public function bz_category_sticky_is_on()
    {
        if (!is_array($this->bz_sticky_categories) or empty($this->bz_sticky_categories)) {
            ...
            foreach ($this->wp_site_categories as $category) {
                $this->bz_sticky_categories[$category->term_id] = array();
            }
    ...
    {
        if (get_option(WPBZCSP)) {
            delete_option(WPBZCSP);
        }
    }

    public function bz_category_sticky_add_custom_box()
    {
        $supported_post_types = apply_filters('bz_category_sticky:post_type:support', array('post'));
    ...
    public function bz_category_sticky_inner_custom_box()
    {
      ...
      echo '<div class="bz-wp-multiselect" style="height:240px;">';
        ...
        foreach ($this->wp_site_categories as $category) {
            ...
            if ($this->bz_sticky_categories[$category->term_id][$post->ID] == 'sticky') {
    ...
    public function bz_category_sticky_save_postdata()
    {
        global $post;
        ...
        if (is_array($bz_post_sticky_categories)) {
            foreach ($bz_post_sticky_categories as $bz_post_sticky_category) {
                $this->bz_sticky_categories[$bz_post_sticky_category][$post->ID] = 'sticky';
            }
        }

        if (is_array($this->bz_sticky_categories)) {
            foreach ($this->bz_sticky_categories as $key => $bz_sticky_category) {
                if (isset($bz_sticky_category[$post->ID]) and !in_array($key, $bz_post_sticky_categories)) {
                    unset($this->bz_sticky_categories[$key][$post->ID]);
                }
            }
        }
    ...
    public function bz_category_sticky_add_sticky_class($classes)
    {
        global $post;

        if (property_exists($post, 'sticky_in_cat')) {
            $classes[] = 'category_sticky_post';
        }
    ...
    public function bz_category_sticky_filter_output($posts)
    {
        ...
        if (!is_array($this->bz_sticky_categories[$cat_obj->term_id])) {
            return $posts;
        }
        ...
        foreach ($this->bz_sticky_categories[$cat_obj->term_id] as $bz_cat_sticky_post => $val) {
            ...
            if ($sticky_post->post_status === 'publish') {
                $newposts[] = $sticky_post;
            }
        }

        foreach ($posts as $post) {
            if (!isset($this->bz_sticky_categories[$cat_obj->term_id][$post->ID])) {
                $newposts[] = $post;
            }
        }
