...
class WpJqMultiSelect
{
    ...
    public function get_urls()
    {
        ...
        foreach ($parts as $key => $part) {
            if (in_array($part, $plugindir_parts)) {
                unset($parts[$key]);
            }
        }
        ...
        return array(
          'base_url'  => $base_url,
          'js_url'    => path_join($base_url, 'js'),
          'style_url' => path_join($base_url, 'css')
        );
    }
    ...
    public function set_style_url($thestyleurl)
    {
        if ($thestyleurl) {
            ...
            return path_join($this->_urls['style_url'], 'jqui.css');
        }
    }
    ...
    public function enqueue_js()
    {
        ...
        foreach ($this->_elements as $element) {
            $elements['element'.$count] = $element;
            ++$count;
        }
